...
const displayMovements = function (movements, sort = false) {
  containerMovements.innerHTML = ""; // vide la page
...
const createUsernames = (accs) => {
  accs.forEach((acc) => {
    acc.username = acc.owner
      .toLowerCase() // texte en miniscule
      .split(" ") // coupe
      .map((name) => name[0]) // map prend la premiere lettre de chaque élement
      .join(""); // les joins entre eux
  });
};
...
btnLogin.addEventListener("click", (e) => {
  ...
  // Vérifie le compte et affiche le compte connecter, le ? court-circuite si l'user ce trompe dans sa connexion de compte evite un bloquage
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    const myTimer = setInterval(() => {
      ...
      // Affiche les min et les sec
      if (timer <= 0) {
        containerApp.style.opacity = 0;
        // Ferme l'appli
        clearInterval(myTimer);
        // Stop le chrono arriver a 0
        timer = 300000;
        // Relance le chrono à 5 min
      }
    }, 1000);
  }
});
